...
function normalizeSections(sections: SlideSection[]): SlideSection[] {
  ...
  sections.forEach((section) => {
    ...
    chunks.forEach((chunk, index) => {
      finalSlides.push({
        title: chunks.length > 1 ? `${section.title} (${index + 1})` : section.title,
        content: [chunk],
      });
    });
  });
...
export class SlidesSkill {
  name = "slides";
  description = "Generate cinematic PowerPoint presentations";
  ...
  async execute(
    ...
    themeName: "netflix" | "apple" | "cyberpunk" = "netflix"
  ) {
    try {
      ...
      cover.addText(`STYLE: ${themeName.toUpperCase()}`, {
        x: 0.8,
        y: 3.2,
        w: 4,
        h: 0.3,
      ...
      sections.forEach((section, index) => {
        ...
        // LAYOUT 4
          ...
          const bullets = section.content[0].split(".").filter(Boolean).slice(0, 4);
          ...
          bullets.forEach((bullet, i) => {
            ...
            slide.addShape("roundRect", {
              ...
              h: 0.9,
              rectRadius: 0.05,
              fill: { color: i % 2 === 0 ? theme.surface : theme.bg },
      ...
      console.error(error);
      await ctx.reply("Erro ao gerar slides.");
