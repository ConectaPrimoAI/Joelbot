...
// ── Estado por usuário ─────────────────────────────────────
const historico: Record<number, Array<{ role: 'user' | 'assistant'; content: string }>> = {};
...
function modelo(msg: string): string {
...
const PRE_FETCH_RULES: Array<{ keywords: string[]; tag: (msg: string) => string }> = [
    {
...
async function enviar(ctx: Context, texto: string) {
    if (!texto?.trim()) return;
    const MAX = 4000;
    const partes: string[] = [];
    let resto = texto;
    while (resto.length > 0) {
        if (resto.length <= MAX) { partes.push(resto); break; }
    ...
    for (const parte of partes) {
        try {
            ...
            try { await ctx.reply(parte.replace(/[*_`\[\]()~>#+=|{}.!]/g, '')); }
            catch (e: any) { addLog(`❌ Envio falhou: ${e.message}`); }
        }
    }
}

// ── Fluxo principal ────────────────────────────────────────
async function processar(ctx: Context, msgUsuario: string, responderVoz = false) {
    const userId = ctx.from?.id || 0;
    const agora  = Date.now();

    if (cooldown[userId] && agora - cooldown[userId] < COOLDOWN) return;
    ...
    try {
        ...
        for (const rule of PRE_FETCH_RULES) {
            ...
            if (rule.keywords.some(k => l.includes(k))) {
                ...
                if (skill) {
                    ...
                    } catch (e: any) { addLog(`⚠️ Pré-fetch falhou: ${e.message}`); }
                }
        ...
        if (dadosInjetados) {
            ...
            if (skill) {
                ...
                if (limpo) await enviar(ctx, limpo);
                ...
                try {
                    ...
                    if (res && typeof res === 'string' && res.trim()) {
                        ...
                        if (res.text) await enviar(ctx, res.text);
                        if (res.file && fs.existsSync(res.file)) {
                            try {
                                if (res.type === 'video')       await ctx.replyWithVideo({ source: res.file });
                                else if (res.type === 'photo')  await ctx.replyWithPhoto({ source: res.file });
                                else                            await ctx.replyWithDocument({ source: res.file });
                                addLog(`📤 ${path.basename(res.file)} enviado`);
                            } finally {
                                try { fs.unlinkSync(res.file); } catch {}
                            }
                        }
                    }
                } catch (e: any) {
        ...
        if (responderVoz && limpo) {
            ...
            if (audio) {
                try { await ctx.replyWithVoice({ source: audio }); }
                finally { try { fs.unlinkSync(audio); } catch {} }
            }
        }

        // ── 5. Atualizar histórico compacto ───────────────────────
        historico[userId].push(
            { role: 'user',      content: msgUsuario.substring(0, 400) },
            { role: 'assistant', content: (limpo || '[ação executada]').substring(0, 400) }
        );
        if (historico[userId].length > MAX_HIST) historico[userId].splice(0, 2);
...
// ── TTS ────────────────────────────────────────────────────
async function gerarAudio(texto: string, userId: number): Promise<string | null> {
...
bot.command('cancelarlembrete', (ctx) => {
    ...
    if (!id) { ctx.reply('Use: /cancelarlembrete [ID]\nPara ver IDs: /lembretes'); return; }
...
bot.on('photo', async (ctx) => {
    try {
        ...
        const caption = (ctx.message as any).caption?.trim() || 'Descreva esta imagem em detalhes em português.';

        const chat = await (groq.chat.completions.create as any)({
...
bot.on('voice', async (ctx) => {
    ...
    try {
        await ctx.sendChatAction('typing').catch(() => {});
        const fid  = (ctx.message as any).voice?.file_id;
        if (!fid) return;
