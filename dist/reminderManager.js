...
async function verificar() {
    ...
    for (const l of lembretes) {
        if (!l.ativo)
            continue;
        if (l.hora !== hhmm)
            continue; // hora errada
        if (l.ultimoDisparoData === data)
        ...
        if (deveDisparar && botRef) {
            try {
                ...
                await botRef.telegram.sendMessage(l.userId, `⏰ *Lembrete${textoTipo}*\n\n${l.mensagem}${textoExtras}`, { parse_mode: 'Markdown' });
...
// ─── API pública ─────────────────────────────────────────────
export function startReminderManager(bot) {
    botRef = bot;
    carregar();
...
export function adicionarLembrete(dados) {
    carregar();
    const novo = {
        ...dados,
...
export function listarLembretes(userId) {
    carregar();
    return lembretes.filter(l => l.userId === userId && l.ativo);
}
export function cancelarLembrete(id, userId) {
    carregar();
...
export function cancelarTodos(userId) {
    carregar();
    let count = 0;
    lembretes.forEach(l => {
        if (l.userId === userId && l.ativo) {
            l.ativo = false;
            count++;
        }
    });
...
//# sourceMappingURL=reminderManager.js.map