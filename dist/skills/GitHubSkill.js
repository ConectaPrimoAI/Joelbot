...
export class GitHubSkill {
    ...
    canHandle(i) { return i.includes('[SYSTEM_GIT:'); }
    api(token) {
        return axios.create({
    ...
    async execute(params, _ctx) {
        ...
        if (!token)
            return '⚠️ Configure GITHUB_TOKEN no Render.';
        ...
        try {
            ...
            if (acao === 'listar') {
                ...
                const linhas = repos.map((r) => `${r.private ? '🔒' : '🌐'} *${r.full_name}* — ${r.description || 'sem descrição'}\n   ⭐${r.stargazers_count} | 🍴${r.forks_count} | ${r.language || '?'}`).join('\n\n');
            ...
            if (acao === 'repo') {
                ...
                if (!nome)
                    return '⚠️ Informe o nome do repositório.';
            ...
            if (acao === 'deletar_repo') {
                ...
                if (!repo)
                    return '⚠️ Informe o repo. Ex: repo="owner/nome"';
            ...
            if (acao === 'analisar') {
                ...
                const files = contentsRes.data.slice(0, 20).map((f) => `${f.type === 'dir' ? '📁' : '📄'} ${f.name}`).join('\n');
                const langs = Object.entries(langRes.data).map(([l, b]) => `${l}: ${(b / 1000).toFixed(0)}KB`).join(' | ');
                const commits = await api.get(`/repos/${repoOwner}/${repoName}/commits?per_page=5`);
                const lastC = commits.data.map((c) => `• ${c.commit.message.split('\n')[0]} (${c.commit.author.name})`).join('\n');
                return (`🐙 *Análise: ${r.full_name}*\n\n` +
            ...
            if (acao === 'arquivo' || acao === 'ler') {
                ...
                if (!repo || !caminho)
                    return '⚠️ Informe repo e caminho.';
                const [repoOwner, repoName] = repo.includes('/') ? repo.split('/') : [owner, repo];
            ...
            if (acao === 'issues') {
                ...
                if (!res.data.length)
                    return `✅ Nenhuma issue aberta em *${repoName}*.`;
                const issues = res.data.map((i) => `#${i.number} ${i.title} — ${i.user.login}`).join('\n');
            ...
            if (status === 401)
                return '❌ Token GitHub inválido. Verifique GITHUB_TOKEN.';
            if (status === 403)
                return '❌ Sem permissão. Token precisa do scope "repo" e "delete_repo".';
            if (status === 404)
                return `❌ Repositório ou arquivo não encontrado.`;
            if (status === 422)
...
//# sourceMappingURL=GitHubSkill.js.map