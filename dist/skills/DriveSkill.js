...
export class DriveSkill {
    ...
    async execute(params, _ctx) {
        ...
        try {
            ...
            return `❌ Google não autenticado: ${e.message}`;
        }
        ...
        try {
            ...
            if (acao === 'listar') {
                ...
                const query = pasta
                    ...
                    : 'trashed=false';
                const res = await axios.get(`${BASE}/files?q=${encodeURIComponent(query)}&pageSize=20&orderBy=modifiedTime+desc` +
                    `&fields=files(id,name,mimeType,modifiedTime,size,webViewLink)`, { headers: h, timeout: 10000 });
            ...
            if (acao === 'ler') {
                ...
                if (!id)
                    return '⚠️ Informe o ID do arquivo.';
                // Se parece um nome, busca o ID
                let fileId = id;
                if (!id.match(/^[a-zA-Z0-9_-]{20,}$/)) {
                    const search = await axios.get(`${BASE}/files?q=${encodeURIComponent(`name='${id}' and trashed=false`)}&fields=files(id,name)`, { headers: h, timeout: 8000 });
                    const found = search.data.files?.[0];
                    if (!found)
                        return `⚠️ Arquivo "${id}" não encontrado.`;
                    fileId = found.id;
                }
            ...
            if (acao === 'criar' || acao === 'upload') {
                ...
                const pasta = p(bloco, 'pasta');
                if (!nome)
                    return '⚠️ Informe o nome do arquivo.';
            ...
            if (acao === 'compartilhar') {
                ...
                const res = await axios.post(`${BASE}/files/${id}/permissions`, body, { headers: { ...h, 'Content-Type': 'application/json' }, timeout: 10000 });
            ...
            if (acao === 'pasta') {
                ...
                if (!nome)
                    return '⚠️ Informe o nome da pasta.';
                const meta = { name: nome, mimeType: 'application/vnd.google-apps.folder' };
                if (pai)
                    meta.parents = [pai];
                const res = await axios.post(`${BASE}/files`, meta, { headers: { ...h, 'Content-Type': 'application/json' }, timeout: 10000 });
            ...
            if (status === 401)
                return '❌ Token Google expirado.';
            if (status === 403)
                return '❌ Sem permissão no Drive.';
            if (status === 404)
...
//# sourceMappingURL=DriveSkill.js.map