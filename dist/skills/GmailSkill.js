...
function buildRaw(para, de, assunto, corpo, replyToId) {
    const lines = [
        `From: ${de}`,
        `To: ${para}`,
...
export class GmailSkill {
    ...
    canHandle(i) { return i.includes('[SYSTEM_GMAIL:'); }
    async execute(params, ctx) {
        ...
        try {
            ...
            return `❌ Google não autenticado. Configure GOOGLE_ACCESS_TOKEN ou as credenciais OAuth2.\nDetalhes: ${e.message}`;
        }
        ...
        try {
            ...
            if (acao === 'listar') {
                ...
                if (!msgs.length)
                    return `📭 Nenhum e-mail encontrado para "${q}".`;
                const detalhes = await Promise.all(msgs.slice(0, 8).map(async (msg) => {
            ...
            if (acao === 'enviar') {
                ...
                if (!para)
                    return '⚠️ Informe o destinatário. Ex: para="email@exemplo.com"';
            ...
            if (acao === 'responder') {
                ...
                const corpo = p(bloco, 'corpo') || '';
                if (!id)
                    return '⚠️ Informe o ID do e-mail para responder.';
            ...
            if (acao === 'buscar') {
                ...
                if (!query)
                    return '⚠️ Informe o termo de busca. Ex: query="from:joao@exemplo.com"';
                const res = await axios.get(`${BASE}/messages?q=${encodeURIComponent(query)}&maxResults=5`, { headers: h, timeout: 10000 });
                const msgs = res.data.messages || [];
                if (!msgs.length)
                    return `📭 Nenhum e-mail encontrado para "${query}".`;
                const detalhes = await Promise.all(msgs.map(async (msg) => {
                    const d = await axios.get(`${BASE}/messages/${msg.id}?format=metadata&metadataHeaders=From,Subject,Date`, { headers: h, timeout: 8000 });
                    const hdrs = d.data.payload?.headers || [];
                    const get = (n) => hdrs.find((h) => h.name === n)?.value || '?';
                    return `📩 *[${msg.id.slice(-8)}]* ${get('Subject')}\n   De: ${get('From')}`;
            ...
            if (acao === 'deletar') {
                const id = p(bloco, 'id');
                if (!id)
                    return '⚠️ Informe o ID do e-mail.';
            ...
            if (status === 401)
                return '❌ Token Google expirado. Reconfigure GOOGLE_ACCESS_TOKEN.';
            if (status === 403)
                return '❌ Sem permissão para o Gmail. Verifique os escopos OAuth2.';
            return `❌ Erro Gmail (${status || 'sem status'}): ${msg}`;
        }
    }
}
//# sourceMappingURL=GmailSkill.js.map